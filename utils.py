...
void_queue = asyncio.Queue(maxsize=1)
...
if not os.path.exists("./data"):
    os.mkdir("data")
if not os.path.exists("./data/void_talk.txt"):
    open("data/void_talk.txt", "w").close()
if not os.path.exists("./data/void_reply.txt"):
    open("data/void_reply.txt", "w").close()
if not os.path.exists("./data/void_random.txt"):
    open("data/void_random.txt", "w").close()
if not os.path.exists("./data/uncensored_links.txt"):
    open("data/uncensored_links.txt", "w").close()

with open("data/void_talk.txt") as file:
    void_talk_channels = {
        int(k.split()[0]): float(k.split()[1])
        for k in file.read().split('\n')
        if k
    }
with open("data/void_reply.txt") as file:
    void_reply_channels = {
        int(k.split()[0]): float(k.split()[1])
        for k in file.read().split('\n')
        if k
    }
with open("data/void_random.txt") as file:
    void_random_channels = {
...
class voidContext(commands.Context):
    ...
    async def void_send(self, content=None, **kwargs):
        msg = content
...
class voidBot(commands.Bot):
    ...
    async def get_context(self, message, cls=None):
        return await super().get_context(
            message,
            cls=cls if cls is not None else voidContext
        )
...
class MessageCollectionType(enum.Enum):
    TRIGGER_OR_void_RANDOM = 0
    void_TALK = 1
    void_REPLY = 2


async def collect_messages(channel, *, mode, before=None):
    """Collects messages from a channel for void Bot"""
    ...
    async for x in channel.history(limit=50, before=before):
        ...
        if not context.valid:
            ...
            if (
                ...
                mode == MessageCollectionType.TRIGGER_OR_void_RANDOM or
...
async def update_data_files():
    ...
    operation, channel_id, randomness, chance = await void_queue.get()
...
async def clean_unused_channels():
    ...
    for channel in list(void_talk_channels):
        if channel not in all_channels:
            del void_talk_channels[channel]
    for channel in list(void_reply_channels):
        if channel not in all_channels:
            del void_reply_channels[channel]
    for channel in list(void_random_channels):
        if channel not in all_channels:
            del void_random_channels[channel]
    for channel in uncensored_link_channels.copy():
        if channel not in all_channels:
            uncensored_link_channels.remove(channel)

    for file, lst in (
        ("void_talk", void_talk_channels),
        ("void_reply", void_reply_channels),
        ("void_random", void_random_channels),
